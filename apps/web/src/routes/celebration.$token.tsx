...
function GuestCelebrationPage() {
  ...
  const signMessage = useMutation(orpc.signGuestCelebrationMessage.mutationOptions({ onSuccess: () => {
    setMessage("");
    setMessageSent(true);
  } }));
  ...
  return <main className="celebrate-page">
    ...
    <section className="guestbook"><div><p>Leave a little love</p><h2>Guestbook</h2></div><form onSubmit={(event) => { event.preventDefault(); if (message.trim()) signMessage.mutate({ token, message: message.trim() }); }}><textarea value={message} onChange={(event) => { setMessage(event.target.value); setMessageSent(false); }} maxLength={500} placeholder="Write Charlene a short message…" /><button type="submit" disabled={signMessage.isPending || message.trim().length < 2}>{signMessage.isPending ? "Sending…" : "Leave a message"}</button></form>{messageSent ? <p className="guestbook-empty">Thank you — Charlene will receive your lovely note.</p> : null}</section>
  </main>;
}
