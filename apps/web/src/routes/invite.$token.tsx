...
function GuestInvitePage() {
  ...
  const respond = (status: "ATTENDING" | "DECLINED") => updateRsvp.mutate({ token, status }, { onSuccess: () => status === "ATTENDING" ? setCelebrating(true) : setDeclined(true) });
  ...
  return <main className="invitation-page"><div className="ambient ambient-red" /><div className="ambient ambient-blue" /><div className="star-field" aria-hidden="true" />
    ...
    <section className={`invitation-scene ${isOpen ? "is-open" : ""}`}><p className="scene-kicker">A special moment to celebrate</p>
      ...
      <article className="invitation-letter"><div className="letter-corner top-left" /><div className="letter-corner bottom-right" /><div className="letter-rule" /><p className="letter-eyebrow">Class of 2026</p><h1>Graduation Invitation</h1><p className="letter-greeting">Dear {formatName(invitation.data.name)},</p><p className="letter-copy">I am delighted to invite you to celebrate my graduation — the joyful close of one chapter and the beginning of another.</p><div className="event-details"><div><span>Date</span><strong>15/08/2026</strong></div><div><span>Venue</span><strong>To be announced</strong></div></div><p className="letter-close">Your presence would make this joyous occasion even more special. I would be honoured to celebrate it with you.</p><p className="signature">Charlene Mbugua</p><div className="rsvp-panel"><p>{rsvp === "ATTENDING" ? "Your attendance is confirmed!" : rsvp === "DECLINED" ? "We’ll be thinking of you." : "Will you be joining us?"}</p><div><button type="button" className="rsvp-yes" disabled={updateRsvp.isPending || Boolean(rsvp)} onClick={() => respond("ATTENDING")}>Able to attend</button><button type="button" className="rsvp-no" disabled={updateRsvp.isPending || Boolean(rsvp)} onClick={() => respond("DECLINED")}>Unable to attend</button></div></div></article>
    </section>
  </main>;
}
